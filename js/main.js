...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Observador para iniciar contador cuando es visible
  ...
  if (roleEl) {
    const dot1 = document.createElement('span'); dot1.textContent = ' · '; dot1.className = 'dot';
    const dot2 = document.createElement('span'); dot2.textContent = ' · '; dot2.className = 'dot';
    ...
    function typeRole() {
      ...
      if (typing) {
        ...
        if (charIndex === role.length) { typing = false; setTimeout(typeRole, 1800); return; }
      } else {
        typed.textContent = role.slice(0, --charIndex);
        if (charIndex === 0) {
          typing = true;
          roleIndex = (roleIndex + 1) % roles.length;
          setTimeout(typeRole, 400); return;
        }
      }
